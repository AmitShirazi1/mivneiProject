public class TwoThreeTree2Key {
    protected TreeNode1Key root;


    public void updateKey1(TreeNode1Key x) {
        x.setKey1(x.getLeft().setKey1());
        if (x.getMiddle() != null) {
            x.setKey1(x.getMiddle().getKey1());
        }
        if (x.getRight() != null) {
            x.setKey1(x.getRight().getKey1());
        }
    }

    public void updateKey2(TreeNode1Key x) {
        x.setKey2(x.getLeft().getKey2());
        if (x.getMiddle() != null) {
            x.setKey2(x.getMiddle().getKey2());
        }
        if (x.getRight() != null) {
            x.setKey2(x.getRight().getKey2());
        }
    }
    ...
    public boolean isOnTheRight(int
    ...
    public TreeNode1Key insertAndSplit(TreeNode1Key parent, TreeNode1Key child) {
        ...
        if (right == null) {
            if (child.getScore() == middle.getScore()) {
                if (child.getID() > left.getID()) {
                    setChildren(parent, child, left, middle);
                } else if (() && (child.getID() < left.getID()) {
                    setChildren(parent, left, child, middle);
                }
            }/*TODO: What the fuck?!*/
            if (child.getScore() < left.getScore()) {
                setChildren(parent, child, left, middle);
            }
            else if (child.getScore() < middle.getScore()) {
                setChildren(parent, left, child, middle);
            }
            else {
